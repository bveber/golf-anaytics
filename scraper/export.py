...
def download_session_csv(
    page: Page, session: RemoteSession, backup_dir: Path, base_url: str, user_id: int
) -> Path:
    """
    Navigate to the session detail page, click the CSV export button,
    and save to backup_dir/sessions/user_<user_id>/<session_id>.csv.
    ...
    dest = backup_dir / "sessions" / f"user_{user_id}" / f"{session.session_id}.csv"
